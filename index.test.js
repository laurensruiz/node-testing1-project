...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    const input = [{ integer: 4 }, { integer: 3 }, { integer: 2 }]
    const input2 = [{ integer: 1 }, { integer: 4 }, { integer: 2 }]
...
describe('[Exercise 4] Counter', () => {
  ...
  test('[8] the count eventually reaches zero but does not go below zero', () => {
    ...
    // 0
    counter.countDown()
    //one more invocation has to be 0
...
describe('[Exercise 5] Seasons', () => {
  ...
  test('[14] the 40th call of seasons.next returns "spring"', () => {
    //less than 39 because it will still increment up once to the 39th invocation
    for(let i =0 ; i < 39; i++){
      seasons.next()
    }
    expect(seasons.next()).toBe('spring')
...
describe('[Exercise 6] Car', () => {
  ...
  test('[17] refueling allows to keep driving', () => {
    ...
    expect(focus.drive(400)).toBe(1000)
